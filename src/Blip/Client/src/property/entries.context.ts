 ...
 * Provides the 'UmbBlockEntriesContext' that umb-block-list-entry needs to render.
...
export class BlipBlockEntriesContext extends UmbBlockEntriesContext<
	...
	UmbBlockManagerContext<UmbBlockTypeBaseModel, UmbBlockLayoutBaseModel, UmbBlockWorkspaceOriginData>,
...
> {
  ...
  readonly hasExpose = new UmbBooleanState(true).asObservable();
	...
	async create(): Promise<UmbBlockDataObjectModel<UmbBlockLayoutBaseModel> | undefined> {
		return undefined;
	}
	...
	): Promise<UmbBlockWorkspaceOriginData> {
		return originData;
	}
